...
* make things worse, the sewer ducts are arranged as a tree with the root of 
* the tree at the entrance to the network of sewers. This means that in order 
* to get from one transformer to the next, there will be a lot of backtracking 
* through the long and claustrophobic ducts because there are no shortcuts 
* between junctions. Furthermore, it's a Sunday; you only have one available 
* technician on duty to search the sewer network for the bad transformers. Your 
* supervisor wants to know how quickly you can get the power back on; he's so 
* impatient that he wants the power back on the moment the technician okays the
* last transformer, without even waiting for the technician to exit the sewers 
* first.
You will be given three int[]'s: fromJunction, toJunction, and ductLength that 
* represents each sewer duct. Duct i starts at junction (fromJunction[i]) and 
* leads to junction (toJunction[i]). ductlength[i] represents the amount of 
* minutes it takes for the technician to traverse the duct connecting 
* fromJunction[i] and toJunction[i]. Consider the amount of time it takes for 
* your technician to check/repair the transformer to be instantaneous. Your 
* technician will start at junction 0 which is the root of the sewer system.
* Your goal is to calculate the minimum number of minutes it will take for your
* technician to check all of the transformers. You will return an int that 
* represents this minimum number of minutes.
* 
Definition
    
Class:
...
public class PowerOutage {
    ...
    public int estimateTimeOut(int[] fromJunction, int[] toJunction, int[] ductLength)
    {
        ...
        {
            int timeSpan=ductLength[i];
            fullTime += timeSpan * 2;
        }
 
        return fullTime - FindLongest(0, fromJunction, toJunction, ductLength);
    }
 
    private int FindLongest(int start, int[] fromJunction, int[] toJunction, int[] ductLength)
    {
        int duration = 0;
        ...
        for (int i = 0; i < toJunction.length; i++)
        {
            ...
            {
                duration = Math.max(duration, (ductLength[i] + FindLongest(toJunction[i], fromJunction, toJunction, ductLength)));

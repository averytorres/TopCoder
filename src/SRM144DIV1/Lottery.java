...
public class Lottery {
	...
	public String[] sortByOdds(String[] rules){
		...
		String[][] compare = new String[rules.length][2];
		
		for(int i=0;i<rules.length;i++){
			int pos1=0;
			int pos2 =rules[i].indexOf(':');
			String name = getNextString(rules[i],pos1,pos2);
			
			pos1=pos2+2;
			pos2=rules[i].indexOf(' ', pos1);
			int choices = getNextInt(rules[i],pos1,pos2);
			
			pos1=pos2+1;
			pos2=rules[i].indexOf(' ', pos1);
			int blanks = getNextInt(rules[i],pos1,pos2);
			
			pos1=pos2+1;
			pos2=rules[i].indexOf(' ', pos1);
			String sorted = getNextString(rules[i],pos1,pos2);
			
			pos1=pos2+1;
			pos2=rules[i].length();
			String unique = getNextString(rules[i],pos1,pos2);
			
			BigInteger odds = computeOdds(choices,blanks,sorted,unique);
		...
		for(int i=0;i<compare.length;i++){
			for(int j=0;j<compare.length;j++){
				...
				BigInteger num1= new BigInteger(compare[i][1]);
				BigInteger num2= new BigInteger(compare[j][1]);
				...
				if(num1.compareTo(num2)==-1){
					...
					compare[i][0]=compare[j][0];
					compare[i][1]=compare[j][1];
				...
				if(num1.equals(num2)&&compare[i][0].compareToIgnoreCase(compare[j][0])<0){
		...
		String result[] = new String[compare.length];
		
		for(int i=0;i<compare.length;i++){
			
				result[i]=compare[i][0];
		}
		return result;
	}

	private BigInteger computeOdds(int choices, int blanks, String sorted,
			String unique) {

		if(sorted.equals("F")&&unique.equals("F")){
			return new BigInteger(choices+"").pow(blanks);
		}
		else if(sorted.equals("F")&&unique.equals("T")){
		
			return choose(choices,blanks).multiply(fact(blanks));
		}
		else if(sorted.equals("T")&&unique.equals("T")){
			
			return choose(choices,blanks);
		}
		else{
			
			return choose(choices + blanks - 1,blanks);
		}
		
	}

	private String getNextString(String sorted, int pos1, int pos2) {
		
		return sorted.substring(pos1, pos2);
	}
	
	private int getNextInt(String sorted, int pos1, int pos2) {
		
		return Integer.parseInt(sorted.substring(pos1, pos2));
	}
	
	private BigInteger choose(int n, int m){
	...
	private BigInteger fact(int n) {
        ...
        for (int i = 1; i <= n; i++) {
        	...
        	fact = new BigInteger(fact.multiply(new BigInteger(i+"")).toString());// *= i;
        }
        return fact;
    }
	...
	public static void main(String[] args){
		...
		String[] out = {"INDIGO: 93 8 T F","ORANGE: 29 8 F T","VIOLET: 76 6 F F","BLUE: 100 8 T T","RED: 99 8 T T","GREEN: 78 6 F T","YELLOW: 75 6 F F"};
		l.sortByOdds(out);
	}
